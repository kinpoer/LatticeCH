...
for i = 1:length(flag)
%     figure
%     subplot(1,length(flag)+1,i+1);hold on
%     p1 = 0.2*i+0.005;
%     set(gca, 'Position', [p1 0 0.17 0.9]);
...
%     material shiny
    ...
    set(allText, 'FontName', 'Arial', 'FontSize', frs);
    allAxes = findall(fig, 'type', 'axes');
    set(allAxes, 'FontName', 'Arial', 'FontSize', frs);
...
for i = 1:n_V
    ...
    surf(Vtmp.*X, Vtmp.*Y, Vtmp.*Z, Vtmp, ...
        'EdgeColor', 'none', 'FaceColor', 'interp', 'FaceAlpha', Transport(i));
end
title(titlestr);
hold off
% maxx = max(Vtmp.*X);maxy = max(Vtmp.*Y);maxz = max(Vtmp.*Z);
% maxrange = max([max([maxx,maxy]),maxz]);
% xlim([-maxrange maxrange]);ylim([-maxrange maxrange]);zlim([-maxrange maxrange]);
axis equal
end
